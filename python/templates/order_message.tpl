...
import com.euronext.optiq.dd.GroupSizeEncodingDecoder;
...
import com.euronext.optiq.integ.common.IAShortOrder;
...
public class {{className}} extends IAShortOrder {
    ...
    */
    private void init() {
        {% for fld in fields %}
        {% if fld.type == 'String' %}
        {{fld.name}} = new IAFieldT<{{fld.type}}>(IAFieldType_enum.StringType, "{{fld.name}}", null, {{fld.isRequired}}, {{loop.index}});
        {%- elif fld.isSet %}
        {{fld.name}} = new IAFieldT<{{fld.type}}>(IAFieldType_enum.SetType, "{{fld.name}}", new {{fld.type}}(), {{fld.isRequired}}, {{loop.index}});
        {%- elif fld.isEnum %}
        {{fld.name}} = new IAFieldT<{{fld.type}}>(IAFieldType_enum.EnumType, "{{fld.name}}", {{fld.type}}.NULL_VAL, {{fld.isRequired}}, {{loop.index}});
        {%- elif fld.isGroup %}
        {{fld.name}} = new IAFieldT<List<{{fld.type}}>>(IAFieldType_enum.GroupType, "{{fld.name}}", new ArrayList<{{fld.type}}>(), {{fld.isRequired}});
        {%- else %}
        {{fld.name}} = new IAFieldT<{{fld.type}}>(IAFieldType_enum.{{fld.type}}Type, "{{fld.name}}", {{className}}Decoder.{{fld.name}}NullValue(), {{fld.isRequired}}, {{loop.index}});
        {%- endif %}
    ...
    */
    ...
    public {{className}}Decoder getDecoder() {
        ...
        dec.wrap(directBuffer, modifiedMsg.getMessageOffset(), dec.sbeBlockLength(), actingVersion);
    ...
    public void set(SbeRawMessage sbeRawMessage) {
        ...
        dec.wrap(directBuffer, sbeRawMessage.getMessageOffset(), dec.sbeBlockLength(), actingVersion);
    ...
    /**
     * Complete a given long order with a {{className}} Sbe Raw Message
     * @param longOrder the long order to complete
     * @param isFirstMessage true means the given sbeRawMessage is the first received message
     */
    public void complete(IALongOrder longOrder, boolean isFirstMessage) {
        {% for fld in fields %}
        {% if fld.name in longOrderFields -%}
        {% if (fld.isGroup) %}
        if (has{{fld.getCleanName()}}()) {
            longOrder.get{{fld.getCleanName()}}().setValue(get{{fld.getCleanName()}}().getValue());
        {% if className == 'IAOrderNewModify' %}
        } else {
            longOrder.get{{fld.getCleanName()}}().reset();
        }
        {%- else -%}
        }
        {% endif %}
    ...
    {#- Getter/Setter methods definition -#}
    {% for fld in fields %}
    {% if fld.isGroup -%}
    ...
    */
    private void set{{fld.getCleanName()}}(final Map<String, String> keyValMap) {
        {{fld.type}}.fromStr(this.{{fld.name}}, keyValMap);
    }
